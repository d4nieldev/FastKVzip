...
def test_exact_fallback_builds_directed_neighbor_to_token_edges_without_self_edges():
    graph_builder = _builder_class()(k=1, nlist=8)
    z = torch.tensor(
...
def test_k_is_capped_at_other_token_count():
    graph_builder = _builder_class()(k=9, nlist=8)
    topology = graph_builder(torch.eye(3).unsqueeze(0))
...
def test_short_context_uses_exact_fallback(monkeypatch, index_mode):
    ...
    topology = graph_builder(torch.randn(4, 4).unsqueeze(0))
