...
def batch_run(tmp_path, monkeypatch):
    ...
    def run(name, *flags):
        active.clear()
        active.update(examples=[], prefills=[], saves=[], logs=[])
        ...
        wandb_run = SimpleNamespace(
            ...
            log=lambda metrics, step: active["logs"].append((dict(metrics), step)),
...
def test_driver_batches_metrics_schedulers_and_epoch_cadence(batch_run, accumulation, windows):
    ...
    assert [step for _metrics, step in run.logs] == list(range(len(windows) + 2))
    assert run.payload["data_cursor"]["global_step"] == 10
...
@pytest.mark.parametrize("limit", [1, 4, 5, 6, 9])
def test_stop_resume_matches_uninterrupted_training(batch_run, schedule, limit):
    ...
    processed = first.payload["data_cursor"]["global_step"]
    assert limit <= processed <= min(10, limit + 1)
...
def test_step_cadence_uses_updates_and_never_saves_a_partial_batch(batch_run):
    ...
    assert [step for metrics, step in run.logs if "validation/answer_nll" in metrics] == [2, 5, 8]
    assert [(c["optimizer_step"], c["global_step"]) for kind, c in run.saves if kind == "last"] == [
        (2, 4), (4, 7), (6, 10)
    ]
...
def test_legacy_resume_normalizes_only_new_fields_and_keeps_original_order(batch_run):
    ...
    del old["data_cursor"]["optimizer_step"]
