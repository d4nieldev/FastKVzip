...
over the same answer positions, at temperature `1`. The two are alternatives;
they are never combined.

KL mode runs the frozen LLM twice per question: once over the untouched
prefilled cache to obtain the reference distribution, and once over the pruned
cache as usual. The reference forward does not update the cache, needs no extra
prefill, and carries no gradient. Because the pruned cache keeps the full
`_seen_tokens`, both forwards place the query and answer at identical
positions. The divergence is accumulated in float32; half precision cannot
resolve a difference between log-probabilities of that magnitude. The NLL is
still reported in KL mode, in bfloat16, so `train/answer_nll` stays comparable
across both objectives.

KL is averaged over a question's answer tokens and then equally across the
questions in a batch, exactly like the NLL. In KL mode W&B also emits
`train/answer_kl` and `validation/answer_kl`, and both `best.pt` selection and
`ReduceLROnPlateau` follow the token-weighted validation KL rather than the
NLL. Expect roughly 20% slower steps: the reference forward attends over the
whole context rather than the retained fraction. It also raises peak memory by
about 2.4 MB per answer token on top of the reference logits, so re-measure a
pilot before submitting a grid.

Use a separate `--output-dir` for each objective. The W&B run ID is stored per
output directory, so reusing one would merge two objectives into a single run
and overwrite `best.pt` with a best selected under the other loss.
...
shuffling disabled. Every checkpoint also records its `loss`; one written before
`--loss` existed resumes as `nll`. A resume inherits the saved objective without
repeating `--loss`, and switching objective mid-resume is refused before W&B,
the model, or the dataset are loaded. Use `--graph-checkpoint` to start a new run from weights
