...
logged together. New runs use the standard W&B **Step** axis, explicitly set to
...
X-axis settings.
Existing history is not rewritten. If W&B history is already ahead of the next
optimizer update (for example, an old run with separate validation rows or a
checkpoint older than its logged history), resume preserves its append-only
Step numbering and emits a warning. In that compatibility case, native Step is
not the optimizer count; `train/optimizer_step` still records the actual count.
