...
| Reuse `ModelKVzip.prefill()` for chunked graph evaluation. | A callback supplies graph scores after each model chunk. | This keeps the official pruning loop. | Copy the chunk loop into `eval_graph_chunked.py`. | User formulation: “reuse as much as possible.” |
| Treat every newly prefetched chunk as one graph. | The mixer scores only that chunk and keeps its scores. | This matches official incremental scoring and avoids recomputing old chunks. | Rebuild one growing graph from all tokens seen so far. | User formulation: “Each chunk is a graph.” |
| Keep separate whole-context and chunked entry points. | `eval_graph.py` stays whole-context. `eval_graph_chunked.py` performs incremental pruning. | Each protocol remains explicit. | Add one mode flag to one script. | User formulation: “let's have 2 scripts.” |
