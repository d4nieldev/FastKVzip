...
| Store hidden states, scores, token IDs, prefix IDs, identity, length, model ID, and prefill chunk in a full cache; scores-only cache entries set `hidden_by_layer` to null. | The scores-only mode saves disk while preserving strict score/context validation before fresh hidden replay. | Store activations only. | Approved plan. |
| Cache before dataset/wrapper/prefill construction for a full-cache hit; scores-only hits replay hidden states without KVzip scoring. | Full entries avoid all teacher work, while scores-only entries avoid recomputing scores. | Check later. | Implementation-only. |
| Release the teacher after student construction when every configured full-cache file exists. Keep it resident in scores-only mode; rebuild it lazily only for a missing full-cache file. | A fully warm full-cache run does not retain the base LLM in GPU memory, while scores-only replay needs it. | Retain the teacher for every full-cache run or construct the student from configuration without first loading the LLM. | Implementation-only: warm-cache memory evidence. |
